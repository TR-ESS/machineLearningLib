...
do
	...
	python ../command/computeDrKdt.py -i $targetAsset'_'$c'_'$d'_corr.log' -c Avr -s $c -l $d -n $e -p -1 &> $targetAsset'_'$trmiAssetNames'_dt_eval_result_'$c'_'$d'_'$e'.log'
...
echo 'TotalSharpRatio,WinningRage,Short,Long,Observe,AccPL' > $targetAsset'_'$trmiAssetNames'_dt_eval_result.log'
more $targetAsset'_'$trmiAssetNames'_dt_eval_result*log' | grep -v double_scalars | grep -v np.std | grep -v Namespace | grep -v TotalSharp | grep -v ::: | grep -v .log >> $targetAsset'_'$trmiAssetNames'_dt_eval_result.log'
