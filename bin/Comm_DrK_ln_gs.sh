...
fname=`./compositeSQL.cmd 'tickdb.'$targetAssetDB "$assetlist" "$trmidblist" "News" $targetAsset "$columnlist" "sum"`
...
do
	...
	python computeCorrelation.py -i $fname -c "windowTimestamp assetCode Open Last $colName" -s $c -l $d
	...
	python computeDrKmodel.py -i 'LCO_'$c'_'$d'_corr.log' -c Avr -s $c -l $d -n $e -p -1 > $targetAsset'_ln_eval_result_'$c'_'$d'_'$e'.log'
...
echo 'TotalSharpRatio,WinningRage,Short,Long,Observe,AccPL' > $targetAsset'_'$trmiAssetNames'_ln_eval_result.log'
more $targetAsset'_'$trmiAssetNames'_ln_eval_result*log' | grep -v double_scalars | grep -v np.std | grep -v Namespace | grep -v TotalSharp | grep -v ::: | grep -v .log >> $targetAsset'_'$trmiAssetNames'_ln_eval_result.log'
