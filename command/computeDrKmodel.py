...
for i in range(startPos,totalRow-(pNumObserve*2)+1,pNumObserve):
    ...
    bestPos=1
    bestSharp=0
    ### Pich up most high ratio
    pDirection=1
    for x in range(0,len(sharpRatios)):
        if abs(sharpRatios[x][0]) > minval:
            bestPos=x+1
            bestSharp=sharpRatios[x][0]
            if bestSharp > 0:
                pDirection=1
            elif bestSharp < 0:
                pDirection=-1
                ...
                pDirection=0
            minval=abs(sharpRatios[x][0])
    ### find position reverse for negative indices
    #negativeDirection=1
    #for x in range(0,len(negativeIndices)):
    #    if indices[bestPos] == negativeIndices[x][0]:
    #        negativeDirection=negativeIndices[x][1]
    #pDirection = pDirection * negativeDirection

    #print (indices[bestPos]+' Sharp Ratio = '+str(bestSharp))
    ### DRY RUN
    j=testEndAt#+1
    dryStartAt=j
    ...
    ddf=_df[[indices[0],indices[bestPos]]][dryStartAt : dryEndAt+1]
...
print (str(tSharp)+',' +str(mwRate)+','+str(pShort) + ',' + str(pLong)+','+str(pNumObserve)+','+str(tAccPL),flush=True)
